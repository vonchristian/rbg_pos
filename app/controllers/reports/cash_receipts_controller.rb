module Reports
  class CashReceiptsController < ApplicationController
    def index
      ...
      if @user.present?
        @cash_receipts = @user.entries.entered_on(from_date: @from_date, to_date: @to_date)
      else
        @cash_receipts = AccountingModule::Entry.entered_on(from_date: @from_date, to_date: @to_date)
      end
      respond_to do |format|
        format.pdf do
          pdf = Reports::CashReceiptsPdf.new(@from_date, @to_date, @cash_receipts, @user, view_context)
