module AccountingModule
  module AmountsExtension
    def balance(hash={})
      if hash[:from_date] && hash[:to_date] && hash[:recorder_id]
      ...
      elsif hash[:from_date] && hash[:to_date] && hash[:recorder_id].nil?
        from_date = hash[:from_date].kind_of?(DateTime) ? hash[:from_date] : DateTime.parse(hash[:from_date].strftime('%Y-%m-%d 12:00:00'))
        to_date = hash[:to_date].kind_of?(DateTime) ? hash[:to_date] : DateTime.parse(hash[:to_date].strftime('%Y-%m-%d 12:59:59'))
      ...
      elsif hash[:commercial_document_id] && hash[:commercial_document_type]
        where(commercial_document_id: hash[:commercial_document_id]).where(commercial_document_type: hash[:commercial_document_type]).
        sum(:amount)
      else
