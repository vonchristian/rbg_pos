module AccountingModule
  module AmountsExtension
    def balance(hash={})
      first_entry_date = 999.years.ago
      ...
      if commercial_document.present? && commercial_document_type.present? && from_date.present? && to_date.present?
        ...
        left_outer_joins(:entry).where('entries.entry_date' => date_range.start_date..date_range.end_date).
        ...
        date_range = DateRange.new(from_date: from_date, to_date: to_date)
        left_outer_joins(:entry).where('entries.entry_date' => date_range.start_date..date_range.end_date).sum(:amount)
      elsif commercial_document.present? && hash[:commercial_document_type].present?
        left_outer_joins(:entry).where('commercial_document_id' => commercial_document).
