class Registry < ApplicationRecord
  ...
  def create_or_find_supplier(row)
  	Supplier.find_or_create_by(business_name: row[7])
  end
  def create_or_find_stock(row)
    find_product(row).stocks.create!(barcode: row[2], quantity: row[3], unit_cost: row[4], total_cost: row[5], retail_price: row[6], wholesale_price: row[6], supplier: create_or_find_supplier(row))
  end
end
